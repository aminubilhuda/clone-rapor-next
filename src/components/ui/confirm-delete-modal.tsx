...
const DEFAULT_WARNING = 'Data yang dihapus tidak dapat dikembalikan.';
...
export default function ConfirmDeleteModal({
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl premium-shadow-lg w-full max-w-md mx-4 animate-modal-in border border-[rgba(0,0,0,0.04)] relative">
        {deleting && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/70 rounded-2xl backdrop-blur-sm">
            <div className="flex flex-col items-center gap-3">
              ...
              <span className="text-sm font-medium text-[#1A1A2E]">Menghapus...</span>
            </div>
          </div>
        )}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[rgba(0,0,0,0.04)]">
          ...
          <button onClick={onClose} disabled={deleting} className="text-gray-400 hover:text-gray-600 transition disabled:opacity-30">
        ...
        <div className="px-6 py-4">
          ...
          <p className="text-gray-700 text-sm">
            Apakah kamu yakin ingin menghapus <strong>{entityName || 'data'}</strong>?
          </p>
        </div>
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-[rgba(0,0,0,0.04)]">
          <button type="button" onClick={onClose} disabled={deleting} className="px-4 py-2 text-sm font-medium text-[#1A1A2E]/60 bg-[#F8F9FB] rounded-xl hover:bg-[#F8F9FB]/80 border border-[rgba(0,0,0,0.06)] active:scale-[0.98] disabled:opacity-50 transition-all">
            Batal
          </button>
          <button
