...
export default function CetakRaporGuruClient({ data, namaKelas, tahun, semester }: Props) {
  ...
  const toggleAll = useCallback((jenis: JenisRapor) => {
    setSelected((prev) => {
      ...
      const allIds = new Set(filteredSiswa.map((s) => s.id_siswa));
  ...
  const isAllChecked = (jenis: JenisRapor) =>
    filteredSiswa.length > 0 && filteredSiswa.every((s) => selected[jenis].has(s.id_siswa));
    ...
    <div>
      ...
      {filteredSiswa.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-[#6B7280]">Belum ada siswa di kelas {namaKelas}.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl premium-shadow border border-[rgba(0,0,0,0.04)] overflow-hidden">
          <div className="bg-blue-600 text-white px-5 py-3 rounded-t-xl flex items-center justify-between">
            <h5 className="font-semibold text-sm">Daftar Rapor {namaKelas}</h5>
                  ...
                  <tr key={siswa.id_siswa} className="border-b border-[rgba(0,0,0,0.03)] hover:bg-[#F8F9FB] transition-colors">
                    ...
                    <td className="text-center px-3 py-2.5">
                      <button
                        ...
                        disabled={loading === `pelengkap-${siswa.id_siswa}`}
                      ...
                      >
                        {loading === `pelengkap-${siswa.id_siswa}` ? (
                          <svg className="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                          </svg>
                    ...
                    <td className="text-center px-3 py-2.5">
                      <div className="flex items-center justify-center gap-2">
                        <input
                          type="checkbox"
                          checked={selected.tengah_semester.has(siswa.id_siswa)}
                          onChange={() => toggleOne('tengah_semester', siswa.id_siswa)}
                          className="w-4 h-4 rounded border-gray-300 text-amber-600 focus:ring-amber-500 cursor-pointer accent-amber-600"
                        />
                        <button
                          onClick={() => handleCetakSingle('tengah_semester', siswa.id_siswa)}
                          disabled={loading === `tengah_semester-${siswa.id_siswa}`}
                        ...
                        >
                          {loading === `tengah_semester-${siswa.id_siswa}` ? (
                            <svg className="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24">
                              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                            </svg>
                    ...
                    <td className="text-center px-3 py-2.5">
                      <div className="flex items-center justify-center gap-2">
                        <input
                          type="checkbox"
                          checked={selected.semester.has(siswa.id_siswa)}
                          onChange={() => toggleOne('semester', siswa.id_siswa)}
                          className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer accent-blue-600"
                        />
                        <button
                          onClick={() => handleCetakSingle('semester', siswa.id_siswa)}
                          disabled={loading === `semester-${siswa.id_siswa}`}
                        ...
                        >
                          {loading === `semester-${siswa.id_siswa}` ? (
                            <svg className="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24">
                              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                            </svg>
                    ...
                    <td className="text-center px-3 py-2.5">
                      <div className="flex items-center justify-center gap-2">
                        <input
                          type="checkbox"
                          checked={selected.p5bk.has(siswa.id_siswa)}
                          onChange={() => toggleOne('p5bk', siswa.id_siswa)}
                          className="w-4 h-4 rounded border-gray-300 text-red-600 focus:ring-red-500 cursor-pointer accent-red-600"
                        />
                        <button
                          onClick={() => handleCetakSingle('p5bk', siswa.id_siswa)}
                          disabled={loading === `p5bk-${siswa.id_siswa}`}
                    ...
                    <td className="text-center px-3 py-2.5">
                      <button
                        onClick={() => handleCetakSingle('buku_induk', siswa.id_siswa)}
                        disabled={loading === `buku_induk-${siswa.id_siswa}`}
                        className="inline-flex items-center gap-1 text-xs font-medium text-white bg-amber-500 hover:bg-amber-600 rounded-lg px-3 py-1.5 transition-all disabled:opacity-50 active:scale-[0.97]"
                      >
                        {loading === `buku_induk-${siswa.id_siswa}` ? (
                          <svg className="animate-spin w-3 h-3" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                          </svg>
