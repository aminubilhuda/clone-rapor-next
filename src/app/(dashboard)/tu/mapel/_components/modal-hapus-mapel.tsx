...
export default function ModalHapus({ open, onClose, mapel, onConfirm }: ModalHapusProps) {
  const [deleting, setDeleting] = useState(false);

  if (!open || !mapel) return null;
      ...
      <div className="bg-white rounded-2xl premium-shadow-lg w-full max-w-md mx-4 animate-modal-in border border-[rgba(0,0,0,0.04)]">
        ...
        <div className="px-6 py-4">
          ...
          <p className="text-gray-700 text-sm">
            Apakah kamu yakin ingin menghapus <strong>{mapel.nama_mapel}</strong>?
          </p>
        </div>
