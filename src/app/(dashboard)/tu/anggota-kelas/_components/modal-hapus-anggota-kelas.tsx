...
export default function ModalHapus({ open, onClose, anggota, onConfirm }: ModalHapusProps) {
  const [deleting, setDeleting] = useState(false);

  if (!open || !anggota) return null;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 animate-modal-in">
        ...
        <div className="px-6 py-4">
          ...
          <p className="text-gray-700 text-sm">
            Apakah kamu yakin ingin menghapus <strong>{anggota.nama_siswa}</strong> dari kelas <strong>{anggota.nama_kelas}</strong>?
          </p>
        </div>
