...
export default function ModalHapus({ open, onClose, piket, onConfirm }: ModalHapusProps) {
  const [deleting, setDeleting] = useState(false);

  if (!open || !piket) return null;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 animate-modal-in">
        ...
        <div className="px-6 py-4">
          ...
          <p className="text-gray-700 text-sm">
            Apakah kamu yakin ingin menghapus piket <strong>{piket.harian}</strong> untuk <strong>{piket.nama_user}</strong>?
          </p>
        </div>
