...
import { addSiswaOrganisasi, removeSiswaOrganisasi } from '@/lib/actions/organisasi-actions';

interface ModalAnggotaProps {
  open: boolean;
  onClose: () => void;
  organisasi: any;
  siswa: any[];
...
export default function ModalAnggotaOrganisasi({ open, onClose, organisasi, siswa, anggota, tahun, semester }: ModalAnggotaProps) {
  ...
  if (!open || !organisasi) return null;
  ...
  const handleAdd = async () => {
    ...
    for (const idSiswa of leftChecked) {
      ...
      fd.set('id_organisasi', String(organisasi.id_organisasi));
      fd.set('id_siswa', String(idSiswa));
      fd.set('tahun', String(tahun));
      fd.set('semester', String(semester));
      const result = await addSiswaOrganisasi(fd);
      if (result.success) {
  ...
  const handleRemove = async () => {
    ...
    for (const idSiswa of rightChecked) {
      ...
      if (anggotaRow?.id_siswa_organisasi) {
        const result = await removeSiswaOrganisasi(anggotaRow.id_siswa_organisasi);
      ...
      <div className="bg-white rounded-2xl premium-shadow-lg w-full max-w-5xl mx-4 animate-modal-in border border-[rgba(0,0,0,0.04)]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[rgba(0,0,0,0.04)]">
          <h3 className="text-lg font-semibold text-[#1A1A2E]">
            Kelola Anggota — {organisasi.nama_organisasi}
          </h3>
