...
interface ModalOrganisasiProps {
  open: boolean;
  onClose: () => void;
  organisasi: any | null;
...
export default function ModalOrganisasi({ open, onClose, organisasi, onSave }: ModalOrganisasiProps) {
  const [saving, setSaving] = useState(false);
  const isEdit = !!organisasi;
      ...
      <div className="bg-white rounded-2xl premium-shadow-lg w-full max-w-lg mx-4 animate-modal-in border border-[rgba(0,0,0,0.04)]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="px-6 py-4 space-y-4">
            <div>
              <label className="block text-sm font-medium text-[#1A1A2E]/80 mb-1.5">Nama Organisasi</label>
              <input name="nama_organisasi" defaultValue={organisasi?.nama_organisasi ?? ''} required className="w-full bg-[#F8F9FB] border border-[rgba(0,0,0,0.08)] rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-red-500/20 focus:border-[#DC2626] outline-none transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#1A1A2E]/80 mb-1.5">Kode</label>
              <input name="kode" defaultValue={organisasi?.kode ?? ''} required className="w-full bg-[#F8F9FB] border border-[rgba(0,0,0,0.08)] rounded-xl px-3.5 py-2.5 text-sm focus:ring-2 focus:ring-red-500/20 focus:border-[#DC2626] outline-none transition-all" />
            </div>
          </div>
