...
import { updateOrganisasi, deleteOrganisasi, updatePembinaOrganisasi } from '@/lib/actions/organisasi-actions';
import ModalOrganisasi from './modal-organisasi';
import ModalHapusOrganisasi from './modal-hapus-organisasi';
import ModalAnggotaOrganisasi from './modal-anggota-organisasi';

const COLUMNS = [
  { key: '_no', label: 'No.' },
  { key: 'nama_organisasi', label: 'Nama Organisasi' },
  { key: 'pembimbing', label: 'Pembina' },
...
export default function OrganisasiClient({ organisasi, users, refSiswa, siswaOrganisasi, tahun, semester }: OrganisasiClientProps) {
  ...
  const filtered = organisasi.filter((row) =>
  ...
  const closeModals = () => { setModalEdit(false); setModalHapus(false); setModalAnggota(false); setSelected(null); };

  const handleSave = async (formData: FormData) => {
    const result = await updateOrganisasi(formData);
    if (result.success) {
  ...
  const handleDelete = async () => {
    if (!selected) return;
    const result = await deleteOrganisasi(selected.id_organisasi);
    if (result.success) {
    ...
    <>
      <div className="bg-white rounded-xl premium-shadow border border-[rgba(0,0,0,0.04)]">
        <div className="border-b border-[rgba(0,0,0,0.04)] px-6 py-4 flex items-center justify-between">
          <h5 className="font-semibold text-[#1A1A2E]">Daftar Organisasi</h5>
              ...
              <tbody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((row, i) => (
                    <tr key={row.id_organisasi} className="border-b border-[rgba(0,0,0,0.03)] hover:bg-[#F8F9FB] transition-colors">
                      {COLUMNS.map((col) => {
                        if (col.key === '_no') {
                        ...
                        if (col.key === 'pembimbing') {
                          const val = guruOptions.find((o: any) => o.value === row.pembina_user_id) || null;
                          return (
                            <td key={col.key} className="px-4 py-3 min-w-44">
                              <Select
                                instanceId={`pembimbing-${row.id_organisasi}`}
                                options={guruOptions}
                                value={val}
                                onChange={(opt: any) => {
                                  const fd = new FormData();
                                  fd.set('id_organisasi', String(row.id_organisasi));
                                  fd.set('id_user', opt?.value?.toString() || '');
                                  fd.set('tahun', String(tahun));
                                  fd.set('semester', String(semester));
                                  updatePembinaOrganisasi(fd).then((r) => {
                                    if (r.success) showToast('Pembina berhasil diupdate!', 'success');
                                    else showToast(r.error || 'Gagal mengupdate pembina!', 'error');
                                  });
                                }}
      ...
      <ModalAnggotaOrganisasi
        open={modalAnggota}
        onClose={closeModals}
        organisasi={selected}
        siswa={refSiswa}
        anggota={siswaOrganisasi}
        tahun={tahun}
        semester={semester}
      />
    </>
