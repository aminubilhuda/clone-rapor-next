...
import { updatePegawai, deletePegawai } from '@/lib/actions/pegawai-actions';
...
export default function PegawaiClient({
  pegawai,
  refJabatan, refKelamin, refAgama, refKepegawaian, refTugasTambahan,
}: PegawaiClientProps) {
  ...
  const filtered = pegawai.filter((row) =>
  ...
  const handleSave = async (formData: FormData) => {
    const result = await updatePegawai(formData);
    if (result.success) {
  ...
  const handleDelete = async () => {
    if (!selected) return;
    const result = await deletePegawai(selected.id_user);
    if (result.success) {
    ...
    <>
      <div className="bg-white rounded-xl premium-shadow border border-[rgba(0,0,0,0.04)]">
        <div className="border-b border-[rgba(0,0,0,0.04)] px-6 py-4 flex items-center justify-between">
          <h5 className="font-semibold text-[#1A1A2E]">Daftar Pegawai</h5>
              ...
              <tbody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((row, i) => (
                    <tr key={row.id_user ?? i} className="border-b border-[rgba(0,0,0,0.03)] hover:bg-[#F8F9FB] transition-colors">
                      {COLUMNS.map((col) => {
                        if (col.key === '_aksi') {
                          return (
