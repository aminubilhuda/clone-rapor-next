...
export default function ModalHapus({ open, onClose, mapelSiswa, onConfirm }: ModalHapusProps) {
  const [deleting, setDeleting] = useState(false);

  if (!open || !mapelSiswa) return null;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 animate-modal-in">
        ...
        <div className="px-6 py-4">
          ...
          <p className="text-gray-700 text-sm">
            Apakah kamu yakin ingin menghapus mapel pilihan <strong>{mapelSiswa.nama_mapel}</strong> untuk <strong>{mapelSiswa.nama_siswa}</strong>?
          </p>
        </div>
