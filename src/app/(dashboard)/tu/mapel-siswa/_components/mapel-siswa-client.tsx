...
import { updateMapelSiswa, deleteMapelSiswa } from '@/lib/actions/mapel-siswa-actions';
import ModalMapelSiswa from './modal-mapel-siswa';
import ModalHapus from './modal-hapus-mapel-siswa';

const COLUMNS = [
  { key: 'id_mapel_siswa', label: 'ID' },
  { key: 'nama_kelas', label: 'Kelas' },
  { key: 'nama_mapel', label: 'Mata Pelajaran' },
  { key: 'nama_siswa', label: 'Siswa' },
  { key: 'aktif_label', label: 'Aktif' },
  { key: '_aksi', label: 'Aksi' },
];

interface MapelSiswaClientProps {
  data: any[];
  refKelas: any[];
  refMapel: any[];
  refSiswa: any[];
...
export default function MapelSiswaClient({ data, refKelas, refMapel, refSiswa }: MapelSiswaClientProps) {
  ...
  const handleSave = async (formData: FormData) => {
    const result = await updateMapelSiswa(formData);
    if (result.success) {
      showToast('Data mapel siswa berhasil disimpan!', 'success');
      closeModals();
    } else {
  ...
  const handleDelete = async () => {
    if (!selected) return;
    const result = await deleteMapelSiswa(selected.id_mapel_siswa);
    if (result.success) {
      showToast('Data mapel siswa berhasil dihapus!', 'success');
      closeModals();
    } else {
    ...
    <>
      <div className="bg-white rounded-lg shadow border border-gray-200">
        <div className="bg-blue-600 text-white px-5 py-3 rounded-t-lg flex items-center justify-between">
          <h5 className="font-semibold">Daftar Mapel Pilihan Siswa</h5>
              ...
              <tbody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((row) => (
                    <tr key={row.id_mapel_siswa} className="border-b hover:bg-gray-50 transition">
                      {COLUMNS.map((col) => {
                        if (col.key === '_aksi') {
                          return (
