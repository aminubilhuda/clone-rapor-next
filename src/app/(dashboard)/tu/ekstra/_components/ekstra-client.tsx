...
import { updateEkstra, deleteEkstra } from '@/lib/actions/ekstra-actions';
import ModalEkstra from './modal-ekstra';
import ModalHapus from './modal-hapus-ekstra';

const COLUMNS = [
  { key: 'id_eskul', label: 'ID' },
...
export default function EkstraClient({ ekstra }: EkstraClientProps) {
    ...
    <>
      <div className="bg-white rounded-lg shadow border border-gray-200">
        <div className="bg-blue-600 text-white px-5 py-3 rounded-t-lg flex items-center justify-between">
          <h5 className="font-semibold">Daftar Ekstrakurikuler</h5>
              ...
              <tbody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((row) => (
                    <tr key={row.id_eskul} className="border-b hover:bg-gray-50 transition">
                      {COLUMNS.map((col) => {
                        if (col.key === '_aksi') {
                          return (
