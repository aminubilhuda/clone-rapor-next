...
import { updateDeskripsi, deleteDeskripsi } from '@/lib/actions/deskripsi-actions';
import ModalDeskripsi from './modal-deskripsi';
import ModalHapus from './modal-hapus-deskripsi';

const COLUMNS = [
  { key: 'id_deskripsi', label: 'ID' },
...
interface DeskripsiClientProps {
  deskripsi: any[];
}

export default function DeskripsiClient({ deskripsi }: DeskripsiClientProps) {
  ...
  const filtered = deskripsi.filter((row) =>
  ...
  const handleSave = async (formData: FormData) => {
    const result = await updateDeskripsi(formData);
    if (result.success) {
      showToast('Data deskripsi rapor berhasil disimpan!', 'success');
      closeModals();
    } else {
  ...
  const handleDelete = async () => {
    if (!selected) return;
    const result = await deleteDeskripsi(selected.id_deskripsi);
    if (result.success) {
      showToast('Data deskripsi rapor berhasil dihapus!', 'success');
      closeModals();
    } else {
    ...
    <>
      <div className="bg-white rounded-xl premium-shadow border border-[rgba(0,0,0,0.04)]">
        <div className="border-b border-[rgba(0,0,0,0.04)] px-6 py-4 flex items-center justify-between">
          <h5 className="font-semibold text-[#1A1A2E]">Daftar Deskripsi Rapor</h5>
              ...
              <tbody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((row) => (
                    <tr key={row.id_deskripsi} className="border-b border-[rgba(0,0,0,0.03)] hover:bg-[#F8F9FB] transition-colors">
                      {COLUMNS.map((col) => {
                        if (col.key === '_aksi') {
                          return (
