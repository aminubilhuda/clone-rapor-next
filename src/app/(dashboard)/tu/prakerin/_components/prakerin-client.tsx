...
import { updatePrakerin, deletePrakerin, importPrakerin } from '@/lib/actions/prakerin-actions';
import ModalPrakerin from './modal-prakerin';
import ModalHapus from './modal-hapus-prakerin';
import ModalImportPrakerin from './modal-import-prakerin';

const COLUMNS = [
  { key: 'id_prakerin', label: 'No.' },
  { key: 'mitra', label: 'Mitra' },
  { key: 'lokasi', label: 'Lokasi' },
  { key: 'tanggal_mulai', label: 'Tgl Mulai' },
  { key: 'tanggal_akhir', label: 'Tgl Akhir' },
  { key: 'instruktur', label: 'Instruktur' },
  { key: 'nama_user', label: 'Pengguna' },
  { key: '_aksi', label: 'Aksi' },
];

interface PrakerinClientProps {
  prakerin: any[];
  users: any[];
}

export default function PrakerinClient({ prakerin, users }: PrakerinClientProps) {
  ...
  const filtered = prakerin.filter((row) =>
  ...
  const closeModals = () => { setModalEdit(false); setModalHapus(false); setModalImport(false); setSelected(null); };

  const handleSave = async (formData: FormData) => {
    const result = await updatePrakerin(formData);
    if (result.success) {
      showToast('Data prakerin berhasil disimpan!', 'success');
      closeModals();
    } else {
  ...
  const handleDelete = async () => {
    if (!selected) return;
    const result = await deletePrakerin(selected.id_prakerin);
    if (result.success) {
      showToast('Data prakerin berhasil dihapus!', 'success');
      closeModals();
    } else {
    ...
    <>
      <div className="bg-white rounded-xl premium-shadow border border-[rgba(0,0,0,0.04)]">
        <div className="border-b border-[rgba(0,0,0,0.04)] px-6 py-4 flex items-center justify-between">
          <h5 className="font-semibold text-[#1A1A2E]">Daftar Prakerin</h5>
          <div className="flex items-center gap-2">
            <button onClick={() => setModalImport(true)} className="bg-[#F8F9FB] text-[#1A1A2E]/70 px-4 py-2 rounded-xl text-sm font-medium hover:bg-[#F8F9FB]/80 border border-[rgba(0,0,0,0.08)] active:scale-[0.98] transition-all">
              Import Excel
            </button>
              ...
              <tbody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((row, i) => (
                    <tr key={row.id_prakerin} className="border-b border-[rgba(0,0,0,0.03)] hover:bg-[#F8F9FB] transition-colors">
                      {COLUMNS.map((col) => {
                        if (col.key === '_aksi') {
                          return (
                        ...
                        if (col.key === 'tanggal_mulai' || col.key === 'tanggal_akhir') {
                          return <td key={col.key} className="px-4 py-3">{formatDate(row[col.key])}</td>;
                        }
                        if (col.key === 'id_prakerin') {
