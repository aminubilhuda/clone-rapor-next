...
import { updatePrakerin, deletePrakerin } from '@/lib/actions/prakerin-actions';
import ModalPrakerin from './modal-prakerin';
import ModalHapus from './modal-hapus-prakerin';

const COLUMNS = [
  { key: 'id_prakerin', label: 'ID' },
    ...
    <>
      <div className="bg-white rounded-lg shadow border border-gray-200">
        <div className="bg-blue-600 text-white px-5 py-3 rounded-t-lg flex items-center justify-between">
          <h5 className="font-semibold">Daftar Prakerin</h5>
              ...
              <tbody>
                {paginatedData.length === 0 ? (
                  ...
                  paginatedData.map((row) => (
                    <tr key={row.id_prakerin} className="border-b hover:bg-gray-50 transition">
                      {COLUMNS.map((col) => {
                        if (col.key === '_aksi') {
                          return (
